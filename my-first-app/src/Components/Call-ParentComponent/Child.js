    ...
    return (
        <div>
            {/* Calling the Methods By default */}
             {console.log(props.ParentHandle.state.name),props.ParentHandle.meetParent(),props.ParentHandle.withParams('ReadAllProps')}
        </div>
    )
}
function CallWithParam(props){
    return (
        <div>
            <button onClick={()=>props.parentWithParam('naveen')}>Meet Parent With Params</button>
        </div>
    )
}
